fn main() {
    ...
    9-85,8-85
    26-80,26-90";
    // split on lines and trim -> Vec<&str>

    fn split_lines(input_string: &str) -> Vec<&str>{
        let rough_split: Vec<&str> = input_string.trim().lines().collect();
        rough_split.iter().map(|current_line| current_line.trim()).collect()
    }

    // split on , and trim -> Vec<[&str;2]>

    fn split_lines_on_comma(input_strings: Vec<&str>) -> Vec<[&str;2]>{
        let mut outputs: Vec<[&str;2]> = Vec::new();
        for current_string in input_strings{
            let split_vec: Vec<&str> = current_string.split(',').collect();
            outputs.push([split_vec[0],split_vec[1]]);
        }
        outputs
    }

    // split on - and trim then convert to u32 to save iteration time -> Vec<[[u32;2];2]>

    fn split_lines_on_dash(input_string_arrays: Vec<[&str;2]>) -> Vec<[[u32;2];2]>{
        let mut outputs: Vec<[[u32;2];2]> = Vec::new();
        for current_string in input_string_arrays{
            let mut users: Vec<[u32;2]> = Vec::new();
            for current_user in current_string{
                let ends: Vec<&str> = current_user.split('-').collect();
                users.push([ends[0].parse::<u32>().unwrap(), ends[1].parse::<u32>().unwrap()]);
            }

            outputs.push([users[0],users[1]]);
        }
        outputs
    }
    ...
    fn tally_sets_containing_other(pairs_list: Vec<[[u32;2];2]>) -> usize{
        ...
        for pair in pairs_list{
            total = total + check_if_either_set_contains_other(pair[0], pair[1]) as usize;
        }
        total
    }
